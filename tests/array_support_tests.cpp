...
TEST_CASE("Array Support - Path Parsing") {
    SUBCASE("Simple array access") {
        auto parts = parse_path_enhanced("items[0]");
    ...
    SUBCASE("Nested field after array") {
        auto parts = parse_path_enhanced("items[2].name");
    ...
    SUBCASE("Multiple array accesses") {
        auto parts = parse_path_enhanced("lists[1].items[3].value");
    ...
    SUBCASE("Invalid index handling") {
        auto parts = parse_path_enhanced("items[abc]");
...
TEST_CASE("Array Support - Get Array Elements") {
    ...
    SUBCASE("Get entire array") {
        auto result = get_field_enhanced(list, "items");
    ...
    SUBCASE("Get array element") {
        auto result = get_field_enhanced(list, "items[1]");
        REQUIRE(result.has_value());
    ...
    SUBCASE("Get field from array element") {
        auto result = get_field_enhanced(list, "items[0].name");
        REQUIRE(result.has_value());
        CHECK(result->get<std::string>() == "Item A");
        
        auto result2 = get_field_enhanced(list, "items[2].value");
        REQUIRE(result2.has_value());
        CHECK(result2->get<int>() == 30);
    }
    
    SUBCASE("Out of bounds access") {
        auto result = get_field_enhanced(list, "items[5]");
        CHECK_FALSE(result.has_value());
        
        auto result2 = get_field_enhanced(list, "items[10].name");
...
TEST_CASE("Array Support - Set Array Elements") {
    ...
    SUBCASE("Set field in array element") {
        bool success = set_field_enhanced(list, "items[1].name", nlohmann::json("Modified Item"));
        CHECK(success);
    ...
    SUBCASE("Set value in array element") {
        bool success = set_field_enhanced(list, "items[0].value", nlohmann::json(99));
        CHECK(success);
    ...
    SUBCASE("Set with type conversion") {
        bool success = set_field_enhanced(list, "items[2].value", nlohmann::json("42"));
        CHECK(success);
        CHECK(list.items[2].value == 42);
    }
    
    SUBCASE("Out of bounds setting") {
        bool success = set_field_enhanced(list, "items[5].name", nlohmann::json("Should Fail"));
        CHECK_FALSE(success);
...
TEST_CASE("Array Support - Nested Arrays") {
    ...
    SUBCASE("Access nested array elements") {
        ...
        auto result1 = get_field_enhanced(data, "lists[0].description");
        REQUIRE(result1.has_value());
        CHECK(result1->get<std::string>() == "First List");
        
        // Access item in second list
        auto result2 = get_field_enhanced(data, "lists[1].items[2].name");
        REQUIRE(result2.has_value());
        CHECK(result2->get<std::string>() == "Epsilon");
        
        // Access item value in first list
        auto result3 = get_field_enhanced(data, "lists[0].items[1].value");
    ...
    SUBCASE("Modify nested array elements") {
        ...
        bool success1 = set_field_enhanced(data, "lists[1].description", nlohmann::json("Modified List"));
        CHECK(success1);
        CHECK(data.lists[1].description == "Modified List");
        
        // Modify item name in first list
        bool success2 = set_field_enhanced(data, "lists[0].items[0].name", nlohmann::json("Modified Alpha"));
        CHECK(success2);
...
TEST_CASE("Array Support - Edge Cases") {
    ...
    SUBCASE("Empty path") {
        auto result = get_field_enhanced(list, "");
        CHECK_FALSE(result.has_value());
        
        bool success = set_field_enhanced(list, "", nlohmann::json("value"));
        CHECK_FALSE(success);
    }
    
    SUBCASE("Array access on non-array field") {
        auto result = get_field_enhanced(list, "description[0]");
        CHECK_FALSE(result.has_value());
        
        bool success = set_field_enhanced(list, "description[0]", nlohmann::json("value"));
        CHECK_FALSE(success);
    }
